-- LinkUp: avoid INSERT ... RETURNING on support_requests (RLS has INSERT-only, no SELECT)
-- INSERT ... RETURNING requires SELECT policy visibility under SECURITY INVOKER.
-- Run in Supabase Dashboard → SQL Editor → project zijrcnzjdafndjdpltst
-- Does NOT change RLS, table, or delete_account insert rules (still uses client user_id).
